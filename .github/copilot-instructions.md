...
# Copilot Instructions — Full-Stack Clean Architecture

> **Last Updated:** March 18, 2026 — Updated CD architecture to reflect shared API Gateway with path-based routing; added `libraryTarget: 'commonjs2'` webpack requirement; updated Lambda handler pattern with domain prefix stripping.
> **Purpose:** Single source of truth for how we build domains, microservices, frontend features, CD infrastructure, and full-stack workflows in this repo. Detailed change history lives in Git — not this header.

This file is an instruction guide for GitHub Copilot and for engineers. It covers the **backend** (NestJS microservices, domain packages, infrastructure), the **frontend** (Next.js webapp, shared UI library, data-access hooks), and the **deployment pipeline** (Terraform modules, CD workflows, preview environments).
  ...
  contracts/
    common/
      ...
      src/
        ...
        pagination.ts
    user/
      package.json          ← depends on @old-st/user-domain + zod
      project.json          ← tags: ["scope:user", "type:contracts"]
      tsconfig.json
      src/
        index.ts            ← domain barrel (import: @old-st/contracts/user)
        schemas.ts
        event-schemas.ts
    order/
      package.json
      ...
      tsconfig.json
      src/
        index.ts            ← domain barrel (import: @old-st/contracts/order)
        schemas.ts
        event-schemas.ts
    product/
      package.json
      ...
      tsconfig.json
      src/
        index.ts            ← domain barrel (import: @old-st/contracts/product)
        schemas.ts
  ...
  webapp/                         ← Next.js App Router frontend
    src/
      app/                        ← routes (page.tsx per segment)
        layout.tsx                ← root shell: Providers + Sidebar + content
        users/
        products/
        orders/
...
infra/
  bootstrap/                      ← One-time per-account AWS setup (S3 state, OIDC, ECR, IAM)
    ...
    variables.tf
    outputs.tf
  init-runner/                    ← ECS Fargate one-shot container for deploy tasks
    Dockerfile
    entrypoint.mjs
  ...
  modules/                        ← Reusable Terraform child modules
    ...
    s3/                           ← S3 bucket
    lambda-api/                   ← Lambda + API Gateway HTTP API v2
    lambda-worker/                ← Lambda + SQS event source mapping
    init-runner/                  ← ECS Fargate one-shot task for deploy tasks
    webapp/                       ← ECS Fargate + ALB for Next.js
...
19. **Pages are thin orchestrators.** A page component wires hooks, state, and child components — it does not contain table/form markup or direct API calls. Domain tables, forms, and action menus live in `components/{domain}/`.
20. **All API communication goes through `@old-st/client-common` hooks and API clients.** Never call `fetch` or Axios directly from page or component code. Use the domain-specific React Query hooks (`useUsers`, `useOrders`, etc.) and the typed API clients in `client-common/infrastructure/api-clients/`.
21. **UI primitives come from `@old-st/ui`.** Never add raw HTML `<table>`, `<button>`, or `<input>` elements. Use the shared shadcn-style components (`Badge`, `Button`, `Card`, `Table`, `Input`, `Select`). New primitives are added to `packages/ui/src/components/`.
22. **Status-to-badge-variant mapping uses domain constants, not string literals.** The `status-variants.ts` file in `apps/webapp/src/lib/` maps entity statuses to badge variants using enum constants from `@old-st/contracts/{domain}` — never hardcode `'ACTIVE'`, `'PENDING'`, etc.
23. **API client responses must be Zod-parsed.** Every API client method in `client-common` passes a `schema` option to `apiRequest()` so responses are validated at runtime.

### Mobile Rules (Expo / React Native)

24. **Mobile screens are thin orchestrators** (same principle as webapp pages). A screen component wires hooks, state, and child components — it does not contain list item or card markup. Domain lists live in `components/{domain}/`.
25. **All API communication goes through `@old-st/client-common` hooks and API clients** — the same shared data-access layer used by the webapp. Never create mobile-only API clients or hooks. Never call `fetch` directly.
26. **Mobile UI primitives come from `@old-st/mobile-ui`.** Never use raw `View`/`Text` for interactive elements when a shared primitive exists (Badge, Button, Card, Input, Text, Separator). New primitives are added to `packages/mobile-ui/src/components/`.
27. **Status-to-badge-variant mapping in mobile uses domain constants.** The `status-variants.ts` file in `apps/mobile/src/lib/` maps entity statuses to `BadgeVariant` from `@old-st/mobile-ui` using enum constants from `@old-st/contracts/{domain}` — never hardcode strings.
28. **Use `StyleSheet.create()` for all non-trivial styles.** Never use inline style objects in mobile components except for one-off dynamic values. Styles are defined at the bottom of the file.

### CD / Deployment Rules

29. **`.github/service-registry.json` is the single source of truth for both CI and CD.** All Terraform environment roots read this file via `jsondecode(file(...))` and create resources with `for_each`. When adding a new service, update the registry — do not edit `.tf` files for routine service additions.
30. **Never hardcode service names, table names, queue names, or env vars in Terraform.** All resource creation must iterate over `service-registry.json` data. The only Terraform edits needed for a new domain are when an entirely new module type is introduced (rare).
31. **The webapp Dockerfile requires `output: 'standalone'` in `next.config.js`.** The multi-stage Docker build produces a self-contained Node.js server via `next start` in standalone mode. Without this config, the Dockerfile will fail.
32. **Preview environments are parameterized, not file-per-developer.** One `infra/environments/preview/` directory serves all previews. State is isolated via S3 key: `preview/{preview_name}/terraform.tfstate`. Never create per-developer `.tf` files.
33. **AWS authentication in CD uses GitHub OIDC — never store AWS credentials as GitHub secrets.** The bootstrap module creates an OIDC provider and IAM deploy role. Workflows use `aws-actions/configure-aws-credentials@v4` with `role-to-assume`.

### E2E Testing Rules

34. **All E2E-testable webapp components must have `data-testid` attributes.** Tables, table rows, filter selects, create buttons, status badges, and forms must include a `data-testid` prop following the pattern `{domain}s-table`, `{domain}-row-{id}`, `status-filter`, `create-{domain}-btn`, `{domain}-status-badge`, `create-{domain}-form`. See `apps/webapp-e2e/src/utils/selectors.ts` for the canonical list.
...
**Complete example:** See `apps/users/user-api-service/src/modules/user.module.ts` in the codebase, or use the `nestjs-service-layers` skill which generates the full module template.
...
- Requires `@aws-sdk/client-secrets-manager` in workspace root `package.json` (for `SecretsConfig`).
...
- [ ] **CI:** Add an entry to `.github/service-registry.json` — include `name` (Nx project), `distPath`, `domain`, `type`, `handler`, `memorySize`, `timeout`, and `envVars` array. If the service needs Aurora/VPC access, set `requiresVpc: true`
...
- [ ] **CI (Prisma only):** Add `pnpm prisma:{domain}:generate` step (after `Install dependencies`) to `ci-test-all.yml`, `ci-fast-check.yml`, and `ci-test-affected.yml` — without this, webpack fails with "unable to locate libquery_engine-rhel-openssl-3.0.x.so.node glob" when building the service on a fresh runner
- [ ] **CD (Infrastructure):** Add infrastructure entries to `.github/service-registry.json` → `infrastructure` section: `dynamodbTables` (with GSIs), `sqsQueues`, and/or `aurora` entries as needed
...
- [ ] Follow `nx-microservice-scaffold` skill for Nx project files — **skip port registration**; install `@aws-sdk/client-sqs` + `@types/aws-lambda` instead of `@nestjs/swagger` + `@codegenie/serverless-express`
...
- [ ] Update `.env.local.example` to match — use the `generate-env-local` skill (add-missing-keys mode) to produce the new entries
- [ ] Add a `QUEUE_CONFIGS` entry to `scripts/setup-localstack.ts`, then run `pnpm run localstack:setup:force`
...
```
apps/users/user-api-service/src/infrastructure/config/dynamodb.config.ts
```
...
```
packages/user-domain/src/infrastructure/repositories/dynamo-user.repository.ts
```
...
### 4B. Prisma + PostgreSQL Pattern (order-domain)

#### Prisma Config (Singleton Client)

```
apps/orders/order-api-service/src/infrastructure/config/prisma.config.ts
```

Key rules:
- **One Prisma client per process** (singleton).
- Import `PrismaClient` from the domain's generated client path (`@old-st/order-domain/infrastructure` re-exports it).
...
```
packages/order-domain/src/infrastructure/repositories/prisma-order.repository.ts
```
...
```
packages/order-domain/src/infrastructure/prisma/schema.prisma
```
...
```bash
# From packages/order-domain (local only):
npx prisma migrate dev --name describe_change   # Create + apply migration
npx prisma generate                              # Regenerate client after schema changes
```

**Deployed environments:** Aurora is in a private VPC — the CI runner cannot reach it. Migrations are applied via the **ECS init-runner** — a one-shot Fargate container that runs registered deploy tasks (migrations, seeds, custom scripts) from within the VPC. The init-runner is defined in `infra/init-runner/` (Dockerfile + `entrypoint.mjs`) and driven by the `deployTasks` array in `service-registry.json`. Each task entry specifies a `type` (`prisma-migrate`, `custom-script`, `dynamodb-seed`, `s3-init`), plus `secretEnvVars` and `envVars` arrays that the Terraform module resolves automatically. When `deployTasks` is empty, zero ECS resources are created and the CD workflow skips the init-runner job entirely — zero cost. The init-runner reads `AWS_SECRETS_ARN` and a `SECRETS_ALLOW_LIST` env var to resolve only the declared secret keys before dispatching tasks. For Prisma migrations, the container has `prisma` pre-installed and runs `npx prisma migrate deploy --schema=/app/prisma/{domain}/schema.prisma`. Never attempt to run `prisma migrate deploy` from a CI runner against a VPC-private Aurora cluster.
...
```
apps/users/user-api-service/src/presentation/filters/domain-exception.filter.ts
```
...
**Never** include `timestamp` in error responses. **Never** omit the `error` field. This shape must be identical across all services (user, product, order, etc.).

---

## 7. Type Safety and Validation

- **Domain constants** in `packages/user-domain/src/domain/constants`.
...
| `{DOMAIN}_SQS_QUEUE_URL` | `http://sqs.{DEFAULT_REGION}.localhost.localstack.cloud:4566/000000000000/{queue-name}` | Full SQS queue URL consumed by `SqsLocalService` during local polling. LocalStack exposes SQS via the DNS-based format `http://sqs.{region}.localhost.localstack.cloud:4566/000000000000/{queue-name}`. The region segment must match `DEFAULT_REGION`. The `000000000000` is the LocalStack fake account ID. |
| `{DOMAIN}_SQS_QUEUE_NAME` | `{queue-name}` | Physical SQS queue name. Used by `scripts/setup-localstack.ts` to create the queue via `QUEUE_CONFIGS`. |
...
|---|---|---|
| `.github/service-registry.json` | Lists every service with deployment metadata (Nx project name, distPath, domain, handler, memorySize, timeout, envVars, requiresVpc). Used by CI workflows to build/start services AND by Terraform `for_each` to create Lambda functions, API Gateways, SQS event source mappings, DynamoDB tables, Aurora clusters, and SQS queues. | Add to `apiServices` or `eventHandlerServices` array for services; add to `infrastructure.dynamodbTables`, `infrastructure.sqsQueues`, `infrastructure.aurora`, or `infrastructure.s3Buckets` for resources |
...
| **Env var role** | `envVars[]` is a **whitelist** of env var *names* — tells Terraform which vars to inject into each Lambda. Terraform resolves the *values* from its own module outputs (table ARNs, queue URLs, etc.) | Provides the *actual values* for CI — specific ports, LocalStack URLs, E2E table names |
| **Where read** | `jsondecode(file(...))` in every Terraform env root; `node -e` in CI build/start scripts | `grep >> $GITHUB_ENV` in `ci-e2e.yml` |

**Why they can't be one file:** Terraform needs structured metadata (which service gets which Lambda config, memory, timeout, VPC) plus a machine-readable whitelist of env var *names*. CI needs concrete *values* (ports, LocalStack URLs) in a format that `$GITHUB_ENV` understands (flat key=value). The values are fundamentally different — CI uses `http://localhost:3000/api`, Terraform generates `https://xxx.execute-api.../user/api`.
...
| `webapp` | Next.js app → ECS Fargate + ALB | `module.webapp` (webapp module) |
| `infrastructure` | DynamoDB tables, SQS queues, S3 buckets, Aurora databases | Individual `module.dynamodb`, `module.sqs`, `module.s3`, `module.aurora` blocks |
| `deployTasks[]` | One-shot deploy tasks (migrations, seeds, scripts) → ECS init-runner | `module.init_runner` (init-runner module) |
...
2. Adding `pnpm prisma:{domain}:generate` after `Install dependencies` — in `ci-test-all.yml`, `ci-fast-check.yml`, and `ci-test-affected.yml`. Without it, webpack fails to locate the Linux query engine binary on a fresh runner.
...
## 11. Copilot Rules (Do Not Violate)

When Copilot generates code here, it MUST:
...
10. **Always read the relevant skill file(s) via `read_file` before implementing any task that falls within a skill's domain. Never infer skill instructions from context alone.**

### Skills Policy (Must Follow)

Before writing any code, Copilot MUST check whether the task matches one or more available skills. If it does:

1. Use `read_file` to load the full skill instructions from its file path.
...
| Adding infrastructure resources (DynamoDB table, SQS queue, Aurora DB, S3 bucket) to Terraform | `cd-register-service` |
| Creating or managing preview environments | `cd-register-service` |

### Workflow Orchestrators (Guided Prompts)

The template ships workflow orchestrator prompts in `.github/prompts/`. These activate automatically when the user describes a matching composite task. They guide the user through a structured interview, plan execution phases, load the correct skills in order, and validate between phases.

#### Available Workflows

| Prompt file | Triggers on | Skills orchestrated |
|---|---|---|
| `new-domain.prompt.md` | "add a domain", "new bounded context", "scaffold a domain" | 8+ skills across domain + contracts + service + infra |
| `new-feature.prompt.md` | "add a feature", "add endpoint", "extend the domain" | 6–8 skills, skipping unchanged layers |
| `new-service.prompt.md` | "add a microservice", "new API service", "new event handler" | 3 skills for Nx scaffold + layers + infra |
| `full-stack-feature.prompt.md` | "full-stack feature", "backend + frontend", "end to end" | 10+ skills across all layers |
| `new-event-service.prompt.md` | "publish events", "event handler", "saga", "async communication" | 2–6 skills depending on pattern |
| `new-e2e-tests.prompt.md` | "add e2e tests", "playwright tests", "api e2e", "write e2e" | 3 skills: e2e-infrastructure → write-api-e2e-tests → write-webapp-e2e-tests |
...
2. **Phase validation.** After each phase, run `get_errors` on affected files. Fix errors before proceeding.
...
- **Mobile**: Co-located — `.spec.tsx` next to source file.
...
- **Backend services** deploy as **Lambda ZIP packages** (not Docker containers). Each service's `main.ts` already exports a `handler` function via `@codegenie/serverless-express` and has `STAGE === 'local'` branching — no app code changes needed for deployment.
- **Shared API Gateway:** All API services share a **single API Gateway HTTP API v2** per environment. Each Lambda is wired to a path-based route: `ANY /{domain}/{proxy+}`. The Lambda handler strips the `/{domain}` prefix before passing the event to NestJS, so NestJS only sees `/api/{resource}`. The `domain` value comes from the service's `domain` field in `service-registry.json`.
- **Webapp** deploys as **ECS Fargate** container (Docker). Multi-stage Dockerfile at `apps/webapp/Dockerfile` produces a standalone Next.js server.
...
- **Webpack output:** All service `webpack.config.js` files must include `libraryTarget: 'commonjs2'` in the `output` block. Without this, the Lambda runtime cannot resolve the `handler` export.
...
| Add a new Aurora database | **Zero** — add to `infrastructure.aurora` |
| Add env vars to a service | **Zero** — add to the service's `envVars` array |
| Add a new NEXT_PUBLIC_* var | **Near-zero** — add to `webapp.envVars` + Dockerfile `ARG`/`ENV` |
...
infra/
  ...
  init-runner/        ← ECS Fargate one-shot container for deploy tasks (Dockerfile + entrypoint + scripts)
  modules/            ← Reusable child modules (networking, dynamodb, aurora, sqs, s3, api-gateway,
                        lambda-api, lambda-worker, init-runner, webapp, secrets, monitoring)
  environments/       ← Root modules per environment
...
Then set `AWS_ACCOUNT_ID`, `AWS_REGION`, and `PROJECT_NAME` as GitHub repository variables.

### Preview Environments

- Fully isolated per preview (own DynamoDB tables, Aurora cluster, SQS queues, Lambda functions, ECS service).
- State isolation via S3 key: `preview/{preview_name}/terraform.tfstate`.
- Reduced resource sizing (Lambda 256MB, Aurora min 0.5 ACU, ECS 256 CPU / 512 memory).
...
**If anything conflicts with the current implementation, update this file first.**
