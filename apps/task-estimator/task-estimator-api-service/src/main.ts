...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('TASK-ESTIMATOR-API-SERVICE')
        .setDescription('TASK-ESTIMATOR-API-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    Logger.log(`🚀 TASK-ESTIMATOR-API-SERVICE is running on: http://localhost:${port}/api`);
    Logger.log(`🚀 TASK-ESTIMATOR-API-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
}
